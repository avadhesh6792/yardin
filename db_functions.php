...
function db_find_private_chats(){
	global $db_connection;
	...
	$query = " Select users.id user_id, chat_names.id chat_name_id, chat_name, username, lock_status, members_id from chat_names  
	...
	if( $result->num_rows > 0 ){
		...
		while ( $row = $result->fetch_assoc()) {
			...
			if(in_array($user_id, $members_id_arr)){
...
function db_find_all_private_chats(){
...
function db_find_all_chats(){
	...
	if($chat_type == 'private'){
		...
		return db_find_all_private_chats();
...
function db_getMetaDataTesting(){
	$message = $_POST['url'];
	$json = httpMetaData($message);
...
function db_send_group_message(){
	...
	if(isset($_POST) && !empty($_POST))
	{
		...
		$messageType = $postData['messageType'] = $messageType;
		$metaImageUrl = $postData['metaImageUrl'] = '';
		if(!empty($_FILES['image']))
		{
			$img = 'image';
			$filename = $_FILES[$img]['name'];
		...
		else if(isset($messageType) and $messageType == 'gif'){
			$message = $postData['message'] = $message;
			$type = $postData['type'] = 'gif';
		}
		else if(isset($messageType) and $messageType == 'emo'){
			$message = $postData['message'] = $message;
    		...
    		$metaImageUrl = substr($sub,0,strpos($sub,$to));

			$type = $postData['type'] = 'emo';
		}else if(isset($messageType) and $messageType == 'url'){
			$message = $postData['message'] = $message;
			$type = $postData['type'] = 'url';
			...
			$json = httpMetaData($url);
		    $decode = json_decode($json, true);
		    
		    if(isset($decode['result']['status']) and $decode['result']['status'] == 'OK'){
		    	if(isset($decode['meta']['image'])){
		    		$metaImageUrl = $decode['meta']['image'];
			    }
			    else if(isset($decode['meta']['favicon'])){
			    	$metaImageUrl = $decode['meta']['favicon'];
			    }
		...
		$query = "Insert Into `group_chat` (`group_id`, `posted_by`, `message`, `posted_date`, `post_date`, `type`, `metaImageUrl`) values ($group_id, $posted_by, '$message', '$posted_date', '$post_date', '$type', '$metaImageUrl')";
		$status =  $db_connection->query($query);
		...
		if($result->num_rows > 0)
		{
			...
			while($res = $result->fetch_assoc())
			{
				...
				if($res['posted_by'] == $user_id)
				{
					...
					$temp['type'] = $res['type'];
					$temp['metaImageUrl'] = $res['metaImageUrl'];
...
function db_get_all_members(){
	...
	$query = "Select members_id, user_id from chat_names where id = $group_id";
	$result = $db_connection->query($query);
	...
	$query = "Select users.id user_id, username, image, user_status from users where id = $group_owner_id";
...
function db_add_remove_friend_to_private_chat(){
	...
	$group_id = $data['group_id'];
	$flag = $data['flag']; // 1 : add, 0: remove

	$sql = "select members_id from `chat_names` where id = $group_id";
	...
	if($flag == 1){
		$members_id_arr[] = $friend_id; // add friend
		$members_id_json = json_encode($members_id_arr);
	}
	else{
		$key = array_search($friend_id, $members_id_arr);
		unset($members_id_arr[$key]); // remove friend
		$members_id_json = json_encode($members_id_arr);
	}
...
function db_get_user_list(){
	...
	$query = "Select id user_id, username, image profile_pic from users order by rand() limit $limit offset $offset";
	if($username){
		$query = "Select id user_id, username, image profile_pic from users where username like '%$username%' order by rand() limit $limit offset $offset";
	}
	$result = $db_connection->query($query);
	...
	if($result->num_rows > 0){
		...
		while ( $row = $result->fetch_assoc() ) {
			...
			$row['username'] = $row['username'] ? $row['username'] : '';
			$row['profile_pic'] = $row['profile_pic'] ? $row['profile_pic'] : '';

			$query = "Select user_id, members_id from chat_names where id = $private_chat_id";
			$member_row = $db_connection->query($query);

			$members_id = $member_row->fetch_assoc();
			$members_id_json = $members_id['members_id'];
			...
			if( in_array($row['user_id'], $members_id_arr) ){
				$row['private_member'] = 'yes';
			}
			else{
				$row['private_member'] = 'no';
			}

			if($members_id['user_id'] != $row['user_id']){
...
function db_get_private_chat_members($private_chat_id){
	global $db_connection;
	$return = [];
	$members = [];
	...
	$query = "Select members_id from chat_names where id = $private_chat_id";
	$result = $db_connection->query($query);
